...
- **UX audit status:** In progress. P1 request-status state rendering and organiser-note removal are complete; user-facing technical error copy has been simplified across key member surfaces. Recent UX cleanup completed: resource search placeholder simplified, resource file action verb standardised to "View file", add-resource category helper text added, request-access PSY copy relabeled to AHPRA registration number wording, admin self-approval copy clarified, AI-generated disclosure callouts added to quiz list + runner intro/results, calendar export study blocks switched to UTC-safe AEST timing, and schedule filter controls now include exam windows/my exam window states. Remaining UX follow-ups are now narrowed to unresolved taxonomy/channel and deeper flow items.
